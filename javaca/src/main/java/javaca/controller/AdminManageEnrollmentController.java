...
public class AdminManageEnrollmentController {
    ...
    /*
	@RequestMapping(value = "/adminEnrollment/search/{userID}",method = RequestMethod.GET)
	public ModelAndView getID(@PathVariable int userID) {
		
		int rid = userID;
		List<StudentCourse> sclist = scService.findActiveEnrollmentByStuID(rid);
		ModelAndView mav = new ModelAndView("adminEnrollment");
		mav.addObject("list",sclist);		
		return mav;
	}
	*/
	...
	public String saveRegistration(@Valid StudentCourse sc,BindingResult result,ModelMap model,RedirectAttributes ra,HttpSession session) {
		if(result.hasErrors()) {
			return "editEnrollment";
		}
		...
		//since I have trouble with dealing directly with foreign keys,so I just take good use of other variable.
		scService.save(sc);
		return "redirect:/adminEnrollment";
	}
	...
	public List<String> initializeCourses() {
        ...
        for(Course c:clist) {
        	...
        	for(StudentCourse sc:sclist) {
        		if(sc.getCourse().equals(c.getCourseID())) counter++;
        	}
